...
def produceRandomlyDeformedImage(image,image2,image3,image4, label, numcontrolpoints, stdDef):
    sitkImage=sitk.GetImageFromArray(image, isVector=False)
    sitkImage2=sitk.GetImageFromArray(image2, isVector=False)
    sitkImage3=sitk.GetImageFromArray(image3, isVector=False)
    sitkImage4=sitk.GetImageFromArray(image4, isVector=False)
    ...
    outimgsitk2 = resampler.Execute(sitkImage2)
    outimgsitk3 = resampler.Execute(sitkImage3)
    outimgsitk4 = resampler.Execute(sitkImage4)
    outlabsitk = resampler.Execute(sitklabel)

    outimg = sitk.GetArrayFromImage(outimgsitk)
    outimg = outimg.astype(dtype=np.float32)
    outimg2 = sitk.GetArrayFromImage(outimgsitk2)
    outimg2 = outimg2.astype(dtype=np.float32)
    outimg3 = sitk.GetArrayFromImage(outimgsitk3)
    outimg3 = outimg3.astype(dtype=np.float32)
    outimg4 = sitk.GetArrayFromImage(outimgsitk4)
    outimg4 = outimg4.astype(dtype=np.float32)

    outlbl = sitk.GetArrayFromImage(outlabsitk)
    outlbl = (outlbl>0.5).astype(dtype=np.float32)

    return outimg,outimg2,outimg3,outimg4,outlbl
